...
// const fetchWikipediaInvention = async (term) => {
//   try {
...
//       // Smart parsing
//       const inventorMatch = extract.match(/invented by ([A-Z][a-z]+(?:\s[A-Z][a-z]+)?)/);
//       const yearMatch = extract.match(/in (\d{3,4})/);
//       const placeMatch = lowerExtract.match(/(in|at|from)\s([A-Z][a-z]+(?:\s[A-Z][a-z]+)?)/);

//       const inventor = inventorMatch ? inventorMatch[1] : null;
//       const year = yearMatch ? yearMatch[1] : null;
//       const place = placeMatch ? placeMatch[0] : null;
//       const type = data.description || null;

//       const how = lowerExtract.includes('developed') || lowerExtract.includes('created')
//         ? 'Developed through experimentation or engineering'
//         : null;

//       // Build keyPoints dynamically
//       const keyPoints = [];

//     if (inventor) keyPoints.push(`👤 <strong>Invented by:</strong> ${inventor}`);
//     if (year) keyPoints.push(`📅 <strong>Year:</strong> ${year}`);
//       if (place) keyPoints.push(`📍 <strong>Origin:</strong> ${place}`);
//       if (type) keyPoints.push(`🧪 <strong>Type:</strong> ${type}`);
//       if (how) keyPoints.push(`⚙ <strong>How:</strong> ${how}`);

//       keyPoints.push(`🔗 <a href="${data.content_urls.desktop.page}" target="_blank" rel="noopener noreferrer">View on Wikipedia</a>`);
...
//   } else {
//     const wikiResult = await fetchWikipediaInvention(value);
//     if (wikiResult) {
//       setFilteredInventions([wikiResult]);
//     } else {
//       setFilteredInventions([]);
//     }
//   }
// };
...
//               {inv.img && <img src={inv.img} alt={inv.name} />}
//               <h3>{inv.name}</h3>
//               <p>{inv.desc}</p>
//               {/* <ul>
...
//             {selectedInvention.img && <img src={selectedInvention.img} alt={selectedInvention.name} />}
  ...
  const inventions = [
    ...
    },
    {
      ...
      keyPoints: [
        "First achieved in 1942 at Chicago Pile-1.",
        "Basis for nuclear power plants.",
        "Provides cleanbut regulated energy."
      ]
    },
    {
      name: "Smartwatch",
        ...
        <div className="invention-modal-overlay" onClick={() => setSelectedInvention(null)}>
          <div className="invention-modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <ul>
              ...
              {/* Wikipedia link always in modal */}
              <li>
                🔗 <a 
                      href={`https://en.wikipedia.org/wiki/${encodeURIComponent(selectedInvention.name)}`} 
                      target="_blank" 
